...
class NeuSRenderer:
    ...
    def render_core_outside(self, rays_o, rays_d, z_vals, sample_dist, nerf, background_rgb=None):
        ...
        dis_to_center = torch.linalg.norm(pts, ord=2, dim=-1, keepdim=True).safe_clip(1.0, 1e10)
    ...
    def render(self, rays_o, rays_d, near, far, view_dir, perturb_overwrite=-1, background_rgb=None, cos_anneal_ratio=0.0,
        ...
        z_vals_outside = None
        if self.n_outside > 0:
            z_vals_outside = jt.linspace(1e-3, 1.0 - 1.0 / (self.n_outside + 1.0), self.n_samples)
            #z_vals_outside = near + (far - near) * z_vals_outside[None, :]
        ...
        # Background model
        if self.n_outside > 0:
            # print(z_vals.shape, z_vals_outside.shape)
